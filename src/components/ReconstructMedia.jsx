...
const ZOOM_FACTOR = 1.25; // 25% zoom na reconstrução

/**
 * Sub-componente de Tile Modular
 * Conectado exclusivamente ao MotionValue central de `progress` [0, 1].
 * Renderiza o pedaço correspondente da imagem com 25% de zoom no hover.
 */
function ModularTile({ tile, progress, imageSrc, cols, rows, zoom = ZOOM_FACTOR }) {
      ...
      >
        ...
        >
          <img
            ...
            className="w-full h-full object-cover select-none pointer-events-none filter saturate-100 contrast-100 brightness-100"
          />
        </div>
      </motion.div>
    </div>
 ...
 * ReconstructMedia
 *
 * Animação de reconstrução modular da imagem com 25% de zoom (scale: 1.25).
 * - Entrada: Reconstrução progressiva em matriz de tiles com zoom de 25%.
 * - Saída: Desconstrução coordenada e reversível proporcional ao progresso.
 * - Totalmente interruptível sem dessincronização ou flicker.
 */
export function ReconstructMedia({
  ...
  // Normalização de imagem única
  const imageSrc = image || src || initialImage || finalImage;
  ...
  }, [isRevealed, prefersReducedMotion, progress]);

  // Derivações contínuas para camada base e camada de segurança
  const baseOpacity = useTransform(progress, [0, 0.85, 1], [1, 0.25, 0], { clamp: true });
  const baseScale = useTransform(progress, [0, 1], [1, 1.02], { clamp: true });
  const safetyOpacity = useTransform(progress, [0, 0.96, 1], [0, 0, 1], { clamp: true });
  ...
  return (
    ...
    >
      {/* 
        ========================================================================
        1. CAMADA BASE (ESTADO INICIAL: ESCALA 1.0 NORMAL)
        ========================================================================
      */}
      <motion.div
      ...
      >
        <img
          ...
          alt={alt}
          loading={loading}
          className="w-full h-full object-cover filter saturate-[0.90] contrast-[0.98] brightness-[0.92]"
        />
      </motion.div>

      {/* 
        ========================================================================
        2. CAMADA DE SEGURANÇA CONTÍNUA (IMAGEM 100% RECONSTRUÍDA COM ZOOM 1.25)
        ========================================================================
      */}
      <motion.div
        className="absolute inset-0 w-full h-full pointer-events-none"
      ...
      {/* 
        ========================================================================
        3. MATRIZ DE RECONSTRUÇÃO MODULAR (TILES COM ZOOM DE 25%)
        ========================================================================
      */}
